...
function getSelectedContext() {
    ...
    if (selection) {
        const selectedText = selection.getRangeElements()
            ...
            .trim();
        if (selectedText)
            return { selectedText, documentId: doc.getId() };
    }
...
function saveAiSettings(providerMode, openAIApiKey, openAIModel) {
...
function searchManual(query, limit = 5) {
...
function insertCitation(paper, style, mode) {
    const doc = DocumentApp.getActiveDocument();
    ...
    rewriteReferences(result.bibliography.map((entry) => entry.formattedText));
    return result;
}
function insertTextAtCursorOrEnd(text) {
    const doc = DocumentApp.getActiveDocument();
    const cursor = doc.getCursor();
    if (cursor) {
...
function rewriteReferences(entries) {
...
function readRangeElementText(rangeElement) {
